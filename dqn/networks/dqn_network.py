...
from dqn.utils.memory import ReplayMemory
...
class DuelDQNNetwork(Model):
    ...
    def __init__(self, num_actions: int, agent_history_length: int):
        ...
        self.conv3 = Conv2D(filters=64, kernel_size=3, strides=1, kernel_initializer=tf.keras.initializers.VarianceScaling(scale=2.0), activation="relu")
        self.flatten = Flatten()
        self.state_hidden = Dense(512, kernel_initializer=tf.keras.initializers.VarianceScaling(scale=2.0), activation='relu')
        self.advantage_hidden = Dense(512, kernel_initializer=tf.keras.initializers.VarianceScaling(scale=2.0), activation='relu')
        self.state_val = Dense(1, kernel_initializer=tf.keras.initializers.VarianceScaling(scale=2.0))
        self.advantage_val = Dense(num_actions, kernel_initializer=tf.keras.initializers.VarianceScaling(scale=2.0))
    ...
    def call(self, x):
        ...
        h3 = self.conv3(h2)
        h4 = self.flatten(h3)
        s_hidden = self.state_hidden(h4)
        a_hidden = self.advantage_hidden(h4)
        state_val = self.state_val(s_hidden)
        raw_advantage = self.advantage_val(a_hidden)
        advantage_val = raw_advantage - K.mean(raw_advantage, axis=1, keepdims=True)
        Q_vals = state_val + advantage_val
